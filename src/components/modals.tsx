...
export const EditTagModal = ({setModalActive, tagValue, tagId}: {setModalActive: any, tagValue: string, tagId: any}) => {
    const [formErr, setFormErr] = useState<z.ZodFormattedError<
        NewTagForm,
        string
    > | null>(null)
    const handleSubmit = async (e: FormEvent) => {
        ...
        try{
            ...
            console.log("formData", parseFormValue.data)
            axios.post("/api/v1/edit-tag", { id: formData.get("id"), name: parseFormValue.data.tag}, {headers: {'Content-Type': 'application/json'}})
            .then( (res): any => {
                console.log(res.data)
                if (res.data.status == 200){
                    toast.success("Success Saved", {
                ...
                <div className="relative rounded-lg shadow bg-[#212121]">
                    ...
                    <div className="p-4 md:p-5">
                        <form action="#" onSubmit={handleSubmit} method="post" className="flex flex-col justify-between items-center gap-3">
                            ...
                            <input type="text" className="p-2 bg-[#414141] rounded-lg w-full focus:border focus:border-gray-500 focus:outline-none" name="tag" placeholder={tagValue} autoComplete="off" required/>
                            {
                                formErr?.tag && (
                                    <>
                                        {formErr?.tag?._errors.map((err) => (
